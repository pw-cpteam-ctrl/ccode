/**
 * 브라우저 지문 위장 — 인스타가 "자동화 프로그램이네" 하고 막는 걸 줄이기 위한 장치.
 *
 * ⚠️ 이 파일은 같은 저장소의 `sns-report-scraper` 프로젝트에서 실사용으로 검증된 것을
 *    그대로 복사해온 것이다. 불러다 쓰지 않고 복사한 이유는, 이 저장소가 "폴더 하나 =
 *    프로젝트 하나, 서로 참조하지 않는다"는 규칙으로 운영되기 때문 — 한쪽을 옮기거나
 *    지웠을 때 다른 쪽이 이유 없이 깨지는 걸 막으려는 것이다.
 *    위장 내용을 고칠 일이 생기면 두 곳 다 손봐야 한다.
 *
 * 로그인할 때와 수집할 때 위장을 똑같이 맞춰야 한다 — 다르면 "의심스러운 기기"로 찍혀서
 * 세션이 멀쩡한데도 로그인 벽이 계속 뜬다.
 */
