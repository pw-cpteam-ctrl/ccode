...
const SYSTEM_PROMPT = [
  ...
  '참가자 데이터(JSON 배열, 0부터 시작하는 인덱스 기준. 각 항목은 {"본문": "..."} 형태로',
  '실제 댓글/후기 텍스트만 담고 있다)와 이번 회차의 필터링 조건을 받는다.',
  '조건이 여러 개면 "(조건A) AND (조건B) OR (조건C)" 형식으로 이미 명확하게 조합되어',
  '있을 수 있다 — AND는 모두 만족, OR는 하나만 만족해도 통과이며 왼쪽부터 순서대로',
  '묶인 것이니 이 논리 구조를 그대로 따라서 판단해라.',
  '조건을 만족하는 참가자만 골라서 각 참가자별로 판단 근거(reason)를 한 문장으로 적어라.',
...
export default async function handler(req, res) {
  ...
  try {
    const message = await client.messages.create({
      ...
      output_config: { effort: 'medium', format: MATCH_OUTPUT_FORMAT },
    });
