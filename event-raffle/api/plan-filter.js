...
const SYSTEM_PROMPT = [
  ...
  '   작성한다.',
  '2. "정성스럽게 썼는지/진심이 담겼는지/성의있게 썼는지" 같은 "정성 정도"를 묻는',
  '   조건은 AI 판단(비용 발생) 대신 기본적으로 글자 수(길이)로 판단해라 — "참여요",',
  '   "좋아요"처럼 참여 인증 성격의 극히 짧은 댓글은 정성이 없다고 보고, 실제 이유나',
  '   감상을 풀어 쓴 긴 댓글은 정성이 있다고 본다. 예시 데이터의 길이 분포를 보고',
  '   적절한 글자 수 기준(대략 30~40자 이상을 기본값으로 삼되 예시를 보고 조정)을',
  '   정해 "(row) => row[\"본문\"].length >= N" 같은 codeRule로 만들어라. 이런 조건은',
  '   "코드로 판단 불가능한 주관적 조건"이 아니라 코드로 처리 가능한 조건으로',
  '   취급한다 — 완벽한 정확도보다 비용 절감(AI 호출 없이 판단)이 우선이다.',
  '3. 본문에서 같은 단어나 문장이 붙어서(띄어쓰기·구두점 유무와 상관없이) 여러 번',
  '   반복되는 "도배" 성격의 조건("N번 이상 반복", "도배", "복붙" 등 표현)이 있으면,',
  '   정규식을 절대 직접 새로 작성하지 말고, 브라우저에 이미 만들어져 있는 전용',
  '   함수를 그대로 호출하는 형태로만 codeRule을 만들어라:',
  '   (row) => isRepeatSpam(row["본문"], N)',
  '   N에는 조건에서 요구하는 반복 횟수(예: "4번 이상"이면 4)를 그대로 넣는다.',
  '   이 함수는 공백·구두점·기호 제거, 연속 동일 문자 뭉개기(그래서 "ㅋㅋㅋㅋ" 같은',
  '   1글자 반복 감탄 표현은 항상 도배가 아닌 것으로 처리됨), 역참조 반복 탐지까지',
  '   이미 검증된 로직으로 구현되어 있으니 직접 정규식을 짜려고 하지 마라. 다른',
  '   조건과 AND/OR로 묶여 있으면 이 호출을 나머지 codeRule 조각과 &&/||로 결합하면',
  '   된다(예: "(row) => /사진/.test(row[\"본문\"]) || isRepeatSpam(row[\"본문\"], 4)").',
  '   조건이 "반복되지 않는 사람", "도배가 아닌 사람", "도배한 사람은 제외" 처럼',
  '   반대로 표현되어 있으면 앞에 !를 붙여서 !isRepeatSpam(row["본문"], N) 형태로',
  '   만들어라(다른 조건과 AND로 묶여 "N자 이상 쓴 사람 중 도배한 사람 제외" 같은',
  '   식이면 "(row) => row[\"본문\"].length >= 300 && !isRepeatSpam(row[\"본문\"], 4)"',
  '   처럼 결합한다).',
  '4. 그 외에 길이/반복만으로 설명 안 되는 주관적 판단(재미, 특정 정서, 진짜 의미',
  '   이해가 필요한 뉘앙스 등)이 남아있다면 그 부분만(AND/OR 구조 포함) 한국어',
  '   문장으로 정리해라.',
  '5. 조건 전체가 코드로 판단 가능하면 subjectiveCondition은 빈 문자열로 답하고,',
  ...
  '   비워두는 쪽이 안전하다.',
  '',
  '예시(도배 조건 단독일 때 — 반드시 이 형식 그대로, isRepeatSpam 대신 직접 정규식을',
  '작성하면 안 됨):',
  '조건: "본문에서 2글자 이상 단위의 같은 단어나 문장이 붙어서 4번 이상 연속',
  '반복되는 사람"',
  '답: {"codeRule": "(row) => isRepeatSpam(row[\\"본문\\"], 4)", "subjectiveCondition": ""}',
].join('\n');
...
export default async function handler(req, res) {
  ...
  try {
    const message = await client.messages.create({
      ...
      output_config: { effort: 'high', format: PLAN_OUTPUT_FORMAT },
    });
