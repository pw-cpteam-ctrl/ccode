...
- [ ] `twitter.js`, `instagram.js`를 실제 계정으로 돌려보고 검증 — **특히 인스타 좌표 파싱
  ...
  - **`twitter.js` 버그 1건 발견+수정:** `megahousestore` 실제 계정으로 테스트했더니 하루에
    30건 이상 올리는 계정인데 3건만 수집되고 6초 만에 끝남. 원인: 리트윗/고정 게시물은 화면에
    `<time>`이 "리트윗한 시점"이 아니라 "원본 게시물이 처음 올라간 날짜"를 그대로 보여주는데,
    최근 리트윗한 옛날 글 하나가 타임라인에 섞여 있으면 스크롤을 시작하기도 전에 "조회 시작일보다
    오래된 글이 나왔다"고 착각해서 즉시 멈춰버림. **모킹 데이터로는 실제 계정의 실제 타임라인
    구성(리트윗 섞임 여부)을 알 수 없어서 미리 못 잡았던 버그.** `collectOnce()`에서
    `[data-testid="socialContext"]` 라벨이 붙은 리트윗/고정 게시물은 통째로 수집 대상에서
    제외하도록 수정 (좋아요/RT수도 원래 이 계정 성과가 아니라 원본 게시물 것이라 집계 관점에서도
    제외가 맞음).
  - **재검증했더니 여전히 조기 중단 (원본 6건 → 필터 2건).** 진단 로그로 원인 게시물을 특정해서
    확인해보니 **"끌올"**이었음 — 리트윗도 고정도 아니고, **인용이든 멘션(답글)이든 오늘 새 글을
    올리면서 6/25 옛날 글을 다시 딸려오게 하는 방식**. 매번 새로운 패턴(리트윗→인용→멘션...)이
    나올 걸로 보여서, 패턴 하나하나 개별 예외처리 하는 대신 **근본적으로 접근을 바꿈**: 오래된
    게시물 딱 1건 나왔다고 바로 멈추지 않고, **새로 발견된 순서(피드 순서) 기준 연속 3건이 전부
    범위 시작일보다 오래돼야** 진짜로 과거로 넘어갔다고 판단하도록 수정. 끌올된 글 하나가 최신
    글들 사이에 섞여도 연속 3건에 걸리지 않아서 무시되고, 진짜 범위 끝에 도달하면 연속으로
    오래된 글만 나오니 정확히 멈추는 방식. **아직 이 버전으로 재검증 전**
