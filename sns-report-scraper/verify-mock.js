...
check('html-report/stock-report: 직전 스냅샷이 있어도 특정 store만 그때 수집 실패(0건)했으면 "신규" 오표시하면 안 됨', () => {
  // 실제로 있었던 버그: BH가 로그인 게이트에 걸려서 0건 수집된 스냅샷 다음에, 정상 수집된
  // 스냅샷과 비교하면 BH 상품 전체가 진짜 신규가 아닌데도 "신규"로 잘못 표시됐음.
  ...
  assert.ok(html.includes('🥇 (-513)'), 'PW는 정상적으로 판매 추정치가 나와야 함');
  ...
  assert.ok(html.includes('470개'), 'BH는 비교 불가 대신 현재 재고 수치를 그대로 보여줘야 함');
});
