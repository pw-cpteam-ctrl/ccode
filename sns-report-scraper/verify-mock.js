...
check('stock-report: rankStockProducts — 신규(비교 불가) 상품은 순위 없이 "-" 처리용 null', () => {
  const ranked = rankStockProducts([
    { productId: 'A', name: '상품A', stock: 9486, stockDelta: 513 },
    { productId: 'B', name: '상품B', stock: 100, stockDelta: null },
    { productId: 'C', name: '상품C', stock: 9062, stockDelta: 438 },
  ]);
  ...
  assert.strictEqual(b.rank, null, '비교 불가 상품은 순위를 매기면 안 됨(근거 없는 숫자 방지)');
});

check('stock-report: findStockMatch — SNS 상품(ip/line)과 재고 상품명 근사 매칭', () => {
  const ranked = rankStockProducts([
    { productId: 'A', name: '[예약] 은혼 GEM 카무이 ver.2 (재판)', stock: 9486, stockDelta: 513 },
    { productId: 'B', name: '은혼 룩업 미니어처 컬렉션', stock: 9062, stockDelta: 438 },
  ]);
...
check('html-report: SNS 표 우측 매출 매칭 컬럼 + 하단 재고 스냅샷 섹션, 둘 다 있어야 함', () => {
  ...
  const stockComparison = {
    ...
    snapshotCount: 2,
    stores: {
      // X1(은혼) 앞에 판매 추정치가 더 큰 상품 3개를 끼워넣어서 은혼이 4위가 되게 함 —
      // 1~3위는 메달이라 순위인 게 바로 보이지만 4위부터는 숫자만 나오면 변화량이랑 헷갈릴 수
      // 있어서 "4위"처럼 "위"를 붙였는지 검증하려는 목적.
      PW: [
        { productId: 'F1', name: '필러상품1', price: 100000, stock: 100, prevStock: 2000, stockDelta: 1900 },
        { productId: 'F2', name: '필러상품2', price: 100000, stock: 100, prevStock: 1900, stockDelta: 1800 },
        { productId: 'F3', name: '필러상품3', price: 100000, stock: 100, prevStock: 1800, stockDelta: 1700 },
        { productId: 'X1', name: '은혼 GEM 카무이 ver.2', price: 220000, stock: 9486, prevStock: 9999, stockDelta: 513 },
      ],
      BH: [{ productId: 'Y1', name: '은혼 GEM 카무이 세컨드', price: 210000, stock: 470, prevStock: 500, stockDelta: 30 }],
    },
    storeComparable: { PW: true, BH: true },
  };

  const html = buildHtmlReport(report, stockComparison);
  assert.ok(html.includes('📦PW 매출'), '헤더에 매출 컬럼이 SNS 표 안에 있어야 함');
  assert.ok(html.includes('4위 (-513)'), '은혼 GEM이 4위로 밀렸을 때 "4위"처럼 순위인 게 명확하게 보여야 함(그냥 "4"만 있으면 변화량 숫자랑 헷갈림)');
...
check('html-report/stock-report: 직전 스냅샷이 있어도 특정 store만 그때 수집 실패(0건)했으면 "신규" 오표시하면 안 됨 — 초기 한도 추정치로 대체', () => {
  ...
  // 스냅샷과 비교하면 BH 상품 전체가 진짜 신규가 아닌데도 "신규"로 잘못 표시됐음. 사용자가
  // "우리(PW)처럼 역산해서 보여달라"고 요청 → 실제 delta 대신 초기 한도(가장 가까운 1000단위)
  // 추정치로 역산해서 형식을 통일하기로 함.
  ...
  assert.strictEqual(bhProduct.estimatedDelta, 530, '1000 - 470 = 530이 추정 판매량이어야 함');

  const html = buildHtmlReport(report, compared);
  assert.ok(html.includes('🥇 (-513)'), 'PW는 정상적으로(실제 delta) 판매 추정치가 나와야 함');
  ...
  assert.ok(!html.includes('470개'), 'BH도 PW처럼 역산된 델타로 표시돼야 함(원본 재고 숫자 그대로 노출 아님)');
  assert.ok(html.includes('🥇 (-530 추정)'), 'BH는 초기 한도(1000) 대비 역산한 추정 판매량이 PW와 같은 형식(순위+변화량)으로 나와야 함');
});
