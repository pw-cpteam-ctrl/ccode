...
const { buildStockComparison, rankStockProducts, findStockMatch } = require('./stock-report');
...
check('html-report: SNS 표 우측에 매칭된 재고 매출순위 컬럼이 별도 섹션 아니라 같은 표 안에 붙어야 함', () => {
  ...
  assert.ok(!html.includes('재고 스냅샷 (실험적)'), '별도 재고 섹션은 더 이상 없어야 함(SNS 표 안으로 흡수)');
