...
check('stock-report: 스냅샷 1개뿐일 땐 비교 없이 현재값만, 2개면 변화량(판매 추정) 계산', () => {
  ...
  assert.strictEqual(onlyOne.stores.PW[0].totalSoldIsEstimated, true, '기준점이 없으니 총 판매추정치도 추정(초기한도 기반)이어야 함');
  ...
  assert.strictEqual(a.totalSold, 513, '실제 최초 관측(9999) 기준 총 판매추정치');
  assert.strictEqual(a.totalSoldIsEstimated, false, '실제 기준점이 있으니 추정 아님');
  assert.strictEqual(b.totalSoldIsEstimated, true, 'B는 이번에 처음 관측돼서 초기한도 추정으로 대체돼야 함');
...
check('html-report: SNS 표 우측 매출 매칭 컬럼 + 하단 재고 스냅샷 섹션, 둘 다 있어야 함', () => {
  ...
  const stockComparison = {
    ...
    stores: {
      ...
      PW: [
        { productId: 'F1', name: '필러상품1', price: 100000, stock: 100, totalSold: 1900, totalSoldIsEstimated: false },
        { productId: 'F2', name: '필러상품2', price: 100000, stock: 100, totalSold: 1800, totalSoldIsEstimated: false },
        { productId: 'F3', name: '필러상품3', price: 100000, stock: 100, totalSold: 1700, totalSoldIsEstimated: false },
        { productId: 'X1', name: '은혼 GEM 카무이 ver.2', price: 220000, stock: 9486, totalSold: 513, totalSoldIsEstimated: false },
      ],
  ...
  assert.ok(html.includes('4위 513개'), '은혼 GEM이 4위로 밀렸을 때 "4위"처럼 순위인 게 명확하게 보여야 함(그냥 "4"만 있으면 판매량 숫자랑 헷갈림)');
...
check('html-report/stock-report: 직전 스냅샷이 있어도 특정 store만 그때 수집 실패(0건)했으면 "신규" 오표시하면 안 됨 — 초기 한도 추정치로 대체', () => {
  ...
  assert.strictEqual(pwProduct.totalSold, 513, 'PW는 실제 최초 관측(9999) 기준 총 판매추정치가 나와야 함');
  assert.strictEqual(pwProduct.totalSoldIsEstimated, false);
  ...
  assert.strictEqual(bhProduct.totalSoldIsEstimated, true, 'BH는 직전 데이터가 없어서 초기한도 추정으로 대체돼야 함');

  const html = buildHtmlReport(report, compared);
  assert.ok(html.includes('🥇 513개'), 'PW는 정상적으로(실제 최초 관측 기준) 총 판매추정치가 나와야 함');
  ...
  assert.ok(html.includes('🥇 530개*'), 'BH는 초기 한도(1000) 대비 역산한 총 판매추정치가 PW와 같은 형식(순위+개수)으로 나오고 "*"로 추정 표시돼야 함');
});
