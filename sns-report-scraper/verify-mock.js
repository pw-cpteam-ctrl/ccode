...
check('html-report: SNS 표 우측 매출 매칭 컬럼 + 하단 재고 스냅샷 섹션, 둘 다 있어야 함', () => {
  ...
  assert.ok(html.includes('class="sm-rank">4위</span>') && html.includes('class="sm-count">513개'), '은혼 GEM이 4위로 밀렸을 때 "4위"처럼 순위인 게 명확하게 보여야 함(그냥 "4"만 있으면 판매량 숫자랑 헷갈림)');
...
check('html-report/stock-report: 직전 스냅샷이 있어도 특정 store만 그때 수집 실패(0건)했으면 "신규" 오표시하면 안 됨 — 초기 한도 추정치로 대체', () => {
  ...
  assert.ok(html.includes('class="sm-rank">🥇</span>') && html.includes('class="sm-count">514개*'), 'PW도 BH와 같은 형식(순위+개수+"*")으로 나와야 함');
  ...
  assert.ok(html.includes('class="sm-count">530개*'), 'BH는 초기 한도(1000) 대비 역산한 총 판매추정치가 PW와 같은 형식(순위+개수)으로 나오고 "*"로 추정 표시돼야 함');
});
