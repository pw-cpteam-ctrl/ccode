...
;  - notion-config.json : 노션 연동 토큰(개인/팀 전용 비밀값, 넘기면 안 됨)
;  - reports\*       : 내가 뽑은 리포트
;  - .git, .gitignore, PLAN/WORKLOG 등 개발용 파일
Source: "*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs ignoreversion; \
  Excludes: "*.md,*.iss,SNS리포트-설치.exe,x-session.json,instagram-session.json,*-session.json,\chrome-profile\*,notion-config.json,\reports\*,\.git\*,.gitignore,\verify-output\*"
