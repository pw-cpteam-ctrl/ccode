/**
 * 여러 기간을 따로 수집한 뒤(run-megahouse.js를 각 기간마다 실행하면 reports/period-cache/에 자동
 * 저장됨) 그 기간들을 한 화면에 나란히 비교하는 리포트를 만듦. 재수집 없이 캐시만 읽어서
 * 몇 초 안에 끝남.
 *
 * 이어져 있는 기간(공백 없음)을 한 번에 보고 싶으면 이 스크립트가 아니라 run-megahouse.js에 전체
 ...
 * 각 값은 run-megahouse.js 실행 시 자동 저장된 reports/period-cache/<시작일_종료일>.json 파일명과 같아야 함
 * (즉 run-megahouse.js를 그 기간으로 먼저 한 번 실행해서 수집해둔 상태여야 함).
 *
...
function loadPeriod(id) {
  ...
  if (!fs.existsSync(cachePath)) {
    console.error(
      `❌ 캐시 없음: ${cachePath}\n` +
      `   "${id}" 기간을 run-megahouse.js로 아직 수집 안 한 것 같음 — 먼저 "node run-megahouse.js ${id.replace('_', ' ')}"로 수집해야 함`
    );
    process.exit(1);
  }
