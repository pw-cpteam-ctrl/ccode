...
const GENERIC_KEYWORDS = [
  ...
  // 굿스마일 브랜드에서 게시물마다 반복되는 이름(메가하우스에서 '메가하우스'를 뺀 것과 같은
  // 이유 — 브랜드명 자체는 상품을 구분해주지 않음). 실제 게시물을 한두 번 수집해본 뒤
  // 반복 상용구를 여기에 더 채워야 함(2026-09 시점엔 굿스마일 데이터가 아직 없음).
  '굿스마일', '굿스마', 'GOODSMILE', 'GOOD SMILE', 'GSC',
  ...
  '원형', '최초', '추가', '추후', '색채', '조형',
  // 자사가 "[예약시작] OO 룩업 ... 초회특전 SET(방석) ... 발매:OO 예약판매 기간 내 5% 할인
  // 캠페인 중!" 형태로 여러 프랜차이즈(페르소나3/헌터x헌터 등) 발표 게시물마다 똑같이 쓰는
  // 고정 템플릿 문구 — 안 걸러내면 서로 다른 프랜차이즈 게시물끼리 "초회특전(방석) SET" 세
  // 단어가 겹친다는 이유만으로 하나로 묶여버림(실제 사례: 페르소나3 리로드 게시물이 헌터x헌터
  // 게시물과 잘못 묶여서, 헌터x헌터 실적처럼 보이던 숫자에 페르소나3 실적이 섞여 있었음).
  '초회', '방석', 'SET',
  // 게시물 상세페이지를 통째로 복사(붙여넣기로 수동 추가 기능용)했을 때 딸려오는 사진 첨부
  // 표시 — 상품명과 무관한데, 사진 여러 장 붙은 게시물끼리 이 단어 하나로 우연히 묶이면
  // 안 되니 상용구 취급.
  '이미지',
  // BH(경쟁사)가 "상품화 결정 정보 ... 룩업 시리즈 신제품 제작 결정! 추가 정보 추후 안내
  // 예정" 형태로 여러 캐릭터 발표 게시물마다 똑같이 쓰는 고정 템플릿 문구 — "추가"/"추후"/
  // "정보"/"안내"/"예정"은 이미 있었는데 "결정"/"제작"이 빠져서, 이 두 단어만으로 서로 다른
  // 캐릭터(디지몬 어드벤처/괴수8호/나루토 질풍전/악입문/사이쿠스/도검난무 등) 발표 게시물이
  // 전부 하나로 묶여버림(실제 사례: 이 7개 프랜차이즈가 "악입문" 이름표 하나로 뭉쳐서, 그
  // 안에 섞인 다른 상품 게시물들이 조용히 안 보이게 됨).
  '결정', '제작',
  // BH가 "상품화 결정 & 뒷모습 공개 ... 신제품 제작 결정 & 조형 일부 첫 공개" 변형 템플릿도
  // 반복 사용 — "뒷모습"(후면컷 공개)은 어떤 상품이든 붙을 수 있는 일반적인 사진 설명이라
  // 위와 같은 이유로 서로 다른 캐릭터(나루토 질풍전 vs 괴수8호 등)를 이어붙임.
  '뒷모습',
  // "※ 국내외 사정에 따라 일부 내용/사항이 변동/변경될 수 있습니다" — 예약 안내 디스클레이머가
  // 다상품 공지든 개별 상품 발표든 거의 모든 게시물 끝에 붙어서, 이 문구만으로 완전히 다른
  // 상품들이 줄줄이 엮이는 문제가 있었음(인스타그램 쪽에서 특히 심함 — 게시물 대부분이
  // 하나로 뭉쳐버림).
  '국내외', '사정에', '따라', '변동될', '변경될', '일부',
  // BH가 매달 다른 상품을 경품으로 거는 "룩업 먼슬리"/각종 추첨 이벤트 당첨자 발표 게시물이
  // 반복해서 쓰는 고정 문구 — "당첨자 발표", "진심으로 축하드립니다", "개별 안내 드릴
  // 예정", "앞으로도 다양한 이벤트... 많은 관심과 참여 부탁드립니다. 감사합니다" 등.
  // 상품명이 매달 바뀌어도 이 문구는 그대로라, 완전히 다른 달/다른 상품 발표 게시물끼리
  // 이 문구 하나로 계속 이어붙는 문제가 있었음(인스타그램 쪽에서 특히 심함 — 거의 모든
  // 게시물이 "신규"라는 이름 하나로 뭉쳐버림).
  '발표', '축하', '진심으로', '드립니다', '분께', '증정', '추첨을', '통해',
  '팔로우', '인용', '앞으로도', '많은', '관심', '기대해', '개별', '다양한',
  // "@megahouse_store"/"@Megahouse_BH" 같은 계정 태그가 밑줄(_)때문에 토큰화 시
  // "megahouse"+"store"(또는 "megahouse"+"bh")처럼 별도 영단어로 쪼개지는데, 이 두 개만
  // 겹쳐도(MIN_SHARED_KEYWORDS=2) 그 자체로 "같은 상품"으로 오판단됨 — 사실상 같은 계정이
  // 쓴 모든 게시물끼리 서로 엮이는 셈이라 특히 인스타그램에서 치명적이었음(거의 전 게시물이
  // 하나로 뭉침). '메가하우스'(한글)는 이미 있었지만 영문 표기가 빠져있었음.
  'megahouse', 'store', 'instagram', 'bh',
];
...
// ⚠️ 순서 주의(위 테노히라 사례와 같은 이유): 긴 이름이 짧은 이름을 포함하는 경우 반드시
// 긴 것을 먼저 둬야 함. 특히 굿스마일의 "POP UP PARADE"는 메가하우스의 "POP"(스케일 라인)을
// 문자열로 포함해서, POP이 앞에 있으면 굿스마일 상품이 전부 메가하우스 POP 라인으로 잘못
// 분류되고 그 결과 서로 다른 상품이 같은 라인으로 묶일 수 있음 → 팝업퍼레이드 계열을 POP보다
// 앞에 배치.
...
const LINE_ALIASES = {
  '스케일': 'POP', 'P.O.P': 'POP', 'Look Up': '룩업', 'MegaCat': '메가캣', 'G.E.M': 'GEM',
  'GMG': 'G.M.G', '인사이드 판타지': 'INSIDE FANTASY', '손바닥': '테노히라',
