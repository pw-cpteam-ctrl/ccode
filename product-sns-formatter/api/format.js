...
export default async function handler(req, res) {
  ...
  try {
    ...
    const message = await client.messages.create({
      // 하이쿠(가장 작은 모델)로 쓰다가 소넷으로 올렸다. 이 프롬프트는 "~하지 마라"
      // 형태의 금지 조건이 스무 개가 넘는데, 그걸 끝까지 지키는 건 큰 모델일수록
      // 잘한다. 실제로 없는 설명을 지어내는 사고가 반복돼서 바꿨다.
      model: 'claude-sonnet-5',
      // 소넷은 생각(thinking)이 기본으로 켜져 있고 그 분량도 max_tokens에 포함된다.
      // 1024로 두면 규칙을 따져보는 도중에 잘려서 결과가 안 나온다.
      max_tokens: 8000,
      // effort는 깊게 생각할수록 비싸진다. 이건 창작이 아니라 규칙 적용 작업이라
      // medium으로 충분하고, 모자라면 올리면 된다.
      output_config: { effort: 'medium', format: { type: 'json_schema', schema: RESULT_SCHEMA } },
      // 규칙·고유명사 사전·지시문은 매 요청 똑같다(1만 글자가 넘는다). 캐시에 올려두면
      // 같은 브랜드로 이어서 변환하는 동안 이 부분 값이 1/10로 떨어진다. 그래서 변하지
      // 않는 것은 전부 system으로 빼고, 매번 달라지는 것(작품명·상품정보·추가지시)만
      // 아래 user에 남겼다 — 앞부분이 한 글자라도 바뀌면 캐시가 통째로 무효가 된다.
      system: [{ type: 'text', text: buildStaticPrompt(BRANDS[brand]), cache_control: { type: 'ephemeral' } }],
      messages: [{ role: 'user', content: buildRequestPrompt(productText, extraInstruction, { workKo, workCandidates }) }],
    });
...
function detectAnomaly(productText, result) {
  ...
  // 단순 서식 실수가 아니라 잘못된 정보라서 여기서 막는다.
  ...
  const madeUp = findUnsourcedWords(productText, result);
  if (madeUp.length) {
    return {
      reason: 'unsourced-words',
      message: `AI가 입력 어디에도 없는 표현(${madeUp.join(', ')})을 설명에 지어냈어요.`,
    };
  }
  return null;
}
...
/* ── 근거 없는 설명 날조 검출 ──────────────────────────────────────────────────
   "고속의 레이스트랙을 누비는 레이싱 미쿠가 완성형 피규어로 등장!" — 입력에는 스펙
   라벨 여섯 줄밖에 없는데 앞의 수식이 통째로 창작된 사고가 실제로 있었다.

   원리: 입력이 "라벨: 값" 줄로만 되어 있으면(스크래퍼가 뽑아주는 데이터가 항상 그렇다)
   설명 문장에 들어갈 수 있는 말은 제목·작품명에 이미 나온 단어와 "~로 등장!" 같은
   고정 어구뿐이다. 그 둘 어디에도 뿌리를 못 두는 어절이 있으면 그건 지어낸 것이다.
   제목·작품명을 대조 기준으로 삼는 이유는, 영문 상품명이 결과에서 한글로 바뀌기
   때문에(Racing Miku → 레이싱 미쿠) 입력 원문과는 글자 비교가 안 되기 때문이다.

   입력에 사람이 쓴 캐릭터 메모(자유 서술)가 붙어 있으면 설명에 쓸 수 있는 말이
   훨씬 넓어지므로, 그때는 메모의 단어도 근거로 인정한다. 다만 그 서술이 한국어가
   아니면(영어·일본어 메모) 번역을 거치면서 글자 대조가 무의미해지므로 검사를 건너뛴다 —
   못 잡는 경우를 남길지언정 멀쩡한 결과를 막지는 않는다. */

// 설명 문장에 항상 나올 수 있는 말들. 상품군 이름은 보통 제목에서 오지만, 제목이
// 짧게 끝나는 경우가 있어 흔한 라인업 이름은 여기에도 둔다.
const ALWAYS_ALLOWED = [
  '등장', '재등장', '신제품', '재판', '예약', '판매', '발매', '높이', '제조사', '수주', '마감',
  '시리즈', '피규어', '스케일', '넨도로이드', '팝업퍼레이드', '코어풀', '피그마', '큐포스켓',
  '룩업', '아크릴', '캔뱃지', '스탠드', '세트', '버전', '라운드', '약',
];

// "상품명: ..." 처럼 라벨과 값으로만 이뤄진 줄인지.
const SPEC_LINE = /^\s*[^:\n]{1,14}\s*:\s*\S/;
// 한글 두 글자 이상이 이어지면 사람이 쓴 서술로 본다.
const KOREAN_PROSE = /[가-힣]{2,}/;

function findUnsourcedWords(productText, result) {
  const lines = String(productText || '').split('\n').map(l => l.trim()).filter(Boolean);
  if (!lines.length) return [];

  // 라벨 줄이 아닌 = 사람이 자유롭게 쓴 줄
  const proseLines = lines.filter(l => !SPEC_LINE.test(l));
  // 한국어가 아닌 자유 서술이 섞여 있으면 글자 대조가 성립하지 않는다 → 검사하지 않음
  if (proseLines.some(l => !KOREAN_PROSE.test(l))) return [];

  const res = String(result || '');
  // 근거 풀: 제목【】 + 작품명『』 + 사람이 쓴 한국어 서술 + 고정 어구
  const pool = [
    ...(res.match(/【([^】]*)】/g) || []),
    ...(res.match(/『([^』]*)』/g) || []),
    ...proseLines,
  ...
  const poolWords = pool.split(/[^가-힣A-Za-z0-9]+/).filter(w => w.length >= 2);
  if (!poolWords.length) return [];

  const suspects = [];
  for (const line of descriptionLines(res)) {
    for (const word of line.split(/\s+/)) {
      const core = word.replace(/[^가-힣A-Za-z0-9]/g, '');
      // 한 글자 어절(조사·수사)과 숫자·영문은 검사 대상에서 뺀다 — 오탐만 낸다.
      if (core.length < 2 || !/[가-힣]{2,}/.test(core)) continue;
      // 근거 단어 하나라도 이 어절 안에 들어 있으면 통과한다. 어미·조사가 붙어도
      // ("미쿠가", "피규어로") 어간이 남아 있으므로 이 방식으로 걸러진다.
      if (poolWords.some(w => core.includes(w) || w.includes(core))) continue;
      if (!suspects.includes(word)) suspects.push(word);
    }
  }
  return suspects;
}

// 결과에서 설명 문장 줄만 고른다 — 제목·스펙·해시태그·작품명 줄은 서식이라 대상이 아니다.
function descriptionLines(result) {
  return String(result || '').split('\n').map(l => l.trim()).filter(Boolean).filter(line => {
    if (line.startsWith('#')) return false;                 // 해시태그
    if (/^【.*】$/.test(line)) return false;                 // 제목
    if (/^[^가-힣]*『[^』]*』\s*$/.test(line)) return false;  // 매체분류 + 작품명
    if (SPEC_LINE.test(line)) return false;                 // 높이/발매/제조사 등
    if (/^[＿_\-─—\s]+$/.test(line)) return false;           // 구분선
    if (/^["'“”‘’].*["'“”‘’]$/.test(line)) return false;     // 캐릭터 대사 오프닝(원문 보존)
    return true;
  });
}
...
/* 프롬프트를 두 덩어리로 나눠 조립한다.

   앞쪽(buildStaticPrompt)은 브랜드만 정해지면 매 요청 글자 하나까지 똑같은 부분이다 —
   지시문, 고유명사 사전, 브랜드 규칙. 1만 글자가 넘어서 값의 대부분을 차지하는데,
   이걸 캐시에 올려두면 같은 브랜드로 이어 변환하는 동안 이 부분이 훨씬 싸진다.

   뒤쪽(buildRequestPrompt)은 상품마다 달라지는 부분이다. 캐시는 "앞에서부터 똑같은
   만큼"만 재사용되므로, 변하는 것을 앞에 한 줄이라도 섞으면 그 뒤가 전부 무효가 된다.
   그래서 작품명 안내(상품마다 다름)도 고정부가 아니라 이쪽에 둔다. */
function buildStaticPrompt(brand) {
  return [
...
function buildRequestPrompt(productText, extraInstruction, workInfo = {}) {
