...
function buildPrompt(productText, extraInstruction, brand) {
  return [
    ...
    '시리즈명·캐릭터명·상품 애칭 등 영문 고유명사는 전부 한글로 표기해라 — 영문을 그대로 남겨두지 마라. 한국에 이미 알려진 공식/통용 표기가 있으면 그걸 쓰고(예: "Blue Archive" → "블루아카이브"), 그런 표기를 확신할 수 없으면 지어내지 말고 원어 발음 그대로 소리 나는 대로 한글로 옮겨 적어라(예: "Purple Lollipop" → "퍼플 롤리팝", "Saori" → "사오리"). 즉 "모르면 영어 그대로 둔다"가 아니라 "모르면 발음대로 한글 표기한다"이다.',
    '고유명사를 한글로 옮길 때 절대로 뜻을 창작해서 별명·수식어로 바꿔치기하지 마라 — 오직 소리 나는 대로만 옮겨라. 예를 들어 "Hatsune Miku"는 반드시 "하츠네 미쿠"로만 써라. "초음속 미쿠", "미래소녀 미쿠"처럼 원문에 없는 의미를 상상해서 지어낸 별명으로 바꾸는 것은 절대 금지다. 아래는 특히 자주 나오는 고정 표기이니 그대로 써라: Hatsune Miku=하츠네 미쿠, Nendoroid=넨도로이드, Chocopuni=쵸코푸니, Kaguya=카구야.',
