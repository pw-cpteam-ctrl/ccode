...
<details open>
<summary>2026-08-19 (V2.3 부시로드 ZIP 가져오기 + 작품명 정식표기 사전)</summary>

## 2026-08-19 (V2.3 부시로드 ZIP 가져오기 + 작품명 정식표기 사전)

### 캐러셀 글자
- **캐러셀 하단 흰 글자를 프리젠테이션(Freesentation) 폰트로 변경** — 제목 800(엑스트라볼드),
  부제 700(볼드)로 부제가 한 단계 굵어짐
  (기술: 상업용 무료 SIL OFL 폰트, 구글폰트 미제공이라 눈누 배포 주소를 `@font-face`로 연결,
  실제 사용하는 700/800 두 굵기만 로드)
- **폰트가 늦게 도착해도 기본 폰트로 굳지 않게 처리** — 화면 표시와 PNG 내보내기 양쪽에 적용
  (기술: `ensureCanvasFont`로 그리기 전 로딩 대기, `renderImageStage`·`exportSlidePng`에서 대기)
- **폰트 주소가 막힌 환경에서도 글자가 정상 출력되게 보장** — 4초 넘으면 시스템 폰트로 진행
  (기술: `Promise.race` 시간 제한, `font-family` 폴백에 기존 시스템 폰트 유지)
- **긴 제목 줄바꿈 위치가 어긋나지 않게 수정**
  (기술: `fitWrappedText`의 측정용 폰트를 실제 그릴 폰트와 일치시킴)
...
- **발주서 엑셀이 같이 있으면 상품과 스펙까지 자동 생성** — 상품명(원어·영문)·사이즈·재질·
  원산지·발매·입고예정·수주마감·상품코드
  (기술: .xlsx가 압축 파일인 점을 이용해 라이브러리 추가 없이 표 파싱, 수주마감은 엑셀
  일련번호를 날짜로 변환)
- **엑셀 양식에서 열 순서가 바뀌어도 값이 조용히 비지 않게 처리**
  (기술: 열 위치 고정 대신 라벨 글자(JAN, size, Order Deadline)로 열 탐색, 상품명 일본어·영문
  구분도 위치가 아니라 문자 종류로 판별)
- **엑셀 없이 사진만 오는 메일도 지원** — 상품 1개에 사진을 모두 담고 직접 입력 안내
  (기술: 엑셀 미검출 시 폴백 경로, 폴더명에서 제목 후보 추출)
- **사진과 상품 연결을 자동으로 확정하지 않고 후보만 제시** — 못 고른 사진은 버리지 않고
  첫 상품에 넣은 뒤 몇 장인지 안내
  (기술: 파일명 일본식 약어(can/acsta/key/poster) 매칭, 전체 세트컷(fullset)은 의도적으로 미배정)
- **가져오기 입구를 브랜드별로 하나만 표시** — 굿스마일=폴더, 부시로드=ZIP, 메가하우스=없음
  (기술: `syncImportZones`가 브랜드 라디오 변경 시 두 입구 표시 전환)

### 작품명 정식표기 (신규 기능)
- **한 번 확인한 작품의 한국 정식 제목을 저장해 다음부터 자동 적용** — 같은 작품의 다른
  상품에도 즉시 반영
  (기술: 영문·일문 어느 쪽으로도 조회되는 사전, `lookupWorkTitle`·`saveWorkTitle`)
- **저장된 표기가 없으면 "처음 보는 작품 — 확인 필요"로 표시** — 지어낸 표기가 그대로
  원고로 나가는 것을 차단
  (기술: `workBlockHtml`에서 확정/미확정 분기 렌더링)
- **정식 제목 검색 버튼 추가** — 검색어를 일본어 원제로 넣어 한국 정식 제목에 빨리 닿게 함
  (기술: `workSearchUrl`, 일본어 원제 우선·없으면 영문)
- **원문을 일본어·영어로 전환해서 볼 수 있음** — 기본 일본어, 선택 기억됨
  (기술: `workLocale` localStorage 저장, 한쪽만 있으면 전환 버튼 숨김)
- **표기를 팀 전체가 공유** — 서버 저장이 실패해도 이 브라우저에는 남고 실패 사실을 알림
  (기술: `api/save-work-title.js`가 `data/work-titles.json`에 커밋, localStorage 병행 저장 후
  불러올 때 로컬을 나중에 덮어써 방금 넣은 값 우선)
- **AI가 작품명을 지어내지 않게 규칙 강화** — 확정 표기가 있으면 강제, 없으면 팀이 써온
  표기 74개 중에서 고르게 함
  (기술: `/api/format` 요청에 `workKo`·`workCandidates` 전달, `buildPrompt`에서 조건부 규칙 주입)

### 버그 수정
- **캐러셀 이미지 단이 화면에서 사라진 자리에 그려서 내용이 안 보이고 오류가 나던 문제 수정**
  — 다른 상품 선택이나 원문 언어 전환처럼 기다리는 중에 화면이 다시 그려질 때 발생
