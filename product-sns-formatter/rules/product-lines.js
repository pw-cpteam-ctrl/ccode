...
// 부제는 원고 텍스트(AI가 만든 『작품명』)가 아니라 상품정보 원문(officialText)에서
// 직접 뽑는다 — 『작품명』은 실제 게시될 원고 본문 그 자체라 건드리면 안 되고,
// "이 상품이 어떤 제품군(넨도로이드/피규어/봉제인형 등)인지"는 보통 영문 상품정보에
// 그대로 박혀있어서 사전 매칭으로 충분히 뽑을 수 있다.
//
...
export function matchProductLine(officialText) {
  const text = officialText || '';
  for (const [pattern, label] of PRODUCT_LINES) {
    if (pattern.test(text)) return label;
  }
  return scaleFallbackLabel(text);
}
