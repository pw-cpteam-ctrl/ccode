...
function detectRowTops(ctx, x, imgHeight) {
  ...
  for (let y = 0; y < imgHeight; y++) {
    ...
    if (!isWhite && runStart < 0) {
      ...
      // photo cells are roughly 170-200px tall; reject shorter (text lines) or longer (merged) runs
      if (runLen > 150 && runLen < 200) tops.push(runStart);
      runStart = -1;
    }
  }
  return tops;
...
function detectColStarts(ctx, y, imgWidth) {
  ...
  for (let x = 0; x < imgWidth; x++) {
    ...
    if (!isWhite && runStart < 0) {
      ...
      if (x - runStart > 90) cols.push(runStart);
      runStart = -1;
    }
  }
  return cols;
...
function detectGrid(img) {
  ...
  const sampleCols = [117, 304, 490, 677, 864]; // rough center-of-column x guesses for a 1000px-wide, 5-col sheet
  const rowSets = sampleCols.map((x) => detectRowTops(ctx, x, img.height));
  const rows = rowSets.reduce((a, b) => (b.length > a.length ? b : a));

  const colStarts = rows.length
    ? detectColStarts(ctx, rows[0] + 40, img.width) // +40 to sample mid-photo, avoids edge artifacts
    : [];

  return { cols: colStarts, rows, cardW: 176, cardH: 176 };
}
