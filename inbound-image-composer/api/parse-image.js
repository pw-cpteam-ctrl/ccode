    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          ip: { type: 'string', description: '추정 IP명 (business-rules.md 표기 가이드 기준으로 정규화. 줄임말로 축약하지 말고 정식 명칭 그대로)' },
          price: { type: 'string', description: '가격 텍스트 (예: 44,400원). 없으면 빈 문자열' },
          ship: { type: 'string', description: '배송비 텍스트 (예: 무료배송, 3,000원). 없으면 무료배송으로 추정' },
          tag: { type: 'string', description: '화이트리스트 태그 중 하나(테노히라/메가캣/GEM) 또는 빈 문자열. 확실하지 않으면 빈 문자열' },
...
function buildSystemPrompt({ ipDictHint, tagWhitelist, moodClusters, productLineNames }) {
  ...
  return [
    ...
    '- 규칙 5 (뱅드림 서브밴드): Ave Mujica·MyGO!!!!!·헬로! 해피 월드·RAISE A SUILEN·Morfonica·',
    '  Afterglow·Pastel＊Palettes·Roselia·무겐다이 뮤타입 등은 뱅드림(BanG Dream) 소속 서브밴드다.',
    '  스토어에 따라 "뱅드림"으로 뭉뚱그릴지 서브밴드명 그대로 쓸지는 앱이 나중에 정하니, 너는',
    '  절대 미리 축약하지 말고 서브밴드명을 정확한 표기 그대로 ip에 적어라.',
    '- 규칙 6 (줄임말 금지): 어떤 경우에도 IP명을 한국식 초성/음절 줄임말로 만들지 마라(예: "어떤',
    '  과학의 초전자포"를 "어과초"로 줄이지 말 것). 항상 정식 명칭 전체를 그대로 적어라.',
