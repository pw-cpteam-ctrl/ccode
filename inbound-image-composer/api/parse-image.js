...
function buildPrompt({ expectedCount, ipDictHint, tagWhitelist, moodClusters, layout }) {
  ...
  return [
    ...
    '- 상품명은 보통 "상품 라인명 + 캐릭터명 + | 작품명/소속그룹" 순서로 되어 있다.',
    '  "팝업퍼레이드", "넨도로이드", "피그마", "블라인드박스", "DMM택토리" 같은 상품 라인명은',
    '  IP명이 아니다 — 절대 그걸 ip 필드에 넣지 마라. 진짜 IP명은 보통 맨 뒤 "|" 뒤에 오는',
    '  작품명이나 소속 그룹명이다 (예: "팝업퍼레이드SP 키류인 사츠키 | 킬라킬"이면 ip는 "킬라킬").',
    '  "|"가 없으면 캐릭터명으로 미루어 판단되는 작품명을 써라 (예: "넨도로이드 에반게리온 초호기"면 ip는 "에반게리온").',
