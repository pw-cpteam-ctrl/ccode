 ...
 * - 따라서 프론트에서 3개 단위로 나눠서 여러 번 호출 → 각 배치별 인식 정확도 개선
 * - 이미지 레이아웃이 cardStrip(사진+텍스트를 함께 수직 이어붙임)일 때 특히 효과적 —
 *   텍스트만 보내면 글자가 흐릴 때 AI가 사진을 볼 수 없어 엉뚱한 텍스트를 만들어내는
 *   문제가 있어, 사진도 함께 보내 캐릭터/작품 인식 단서로 쓰게 함
 *
 * 배포 선택사항:
...
function buildPrompt({ expectedCount, ipDictHint, tagWhitelist, moodClusters, layout }) {
  ...
  // cardStrip: 각 상품의 "사진 + 그 아래 텍스트(상품명/가격 등)"를 통째로 잘라 세로로
  // 이어붙인 이미지(빨간 테두리 + #1,#2... 번호로 구분됨). 텍스트가 흐리거나 잘려도 사진을
  // 보고 캐릭터/작품을 유추할 수 있게 하기 위함. 한 번에 너무 많은 항목을 통째로 보내면
  // 항목당 인식 정확도가 급격히 떨어져서(실사용 중 25개 중 4개만 인식되는 문제 발견) 소수
  // 단위로 나눠 보낸다.
  const layoutInstruction = layout === 'cardStrip'
    ? [
      `이미지는 각 상품의 사진과 그 아래 텍스트(상품명/가격 등)를 함께 잘라서 위에서 아래로 이어붙인 것이다.`,
